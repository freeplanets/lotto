...
export default class CreditAccess {
  ...
  ModifyUserCredit(balance: number) {
    return new Promise<IMsg>(async (resolve) => {
      const msg:IMsg = { ErrNo: ErrCode.PASS };
  ...
  async ModifyCredit(money: number, justquery?: boolean):Promise<IMsg> {
    let  msg = await this.getUserCredit();
    if (justquery) {
      msg.orderId = 0;
      return msg;
    }
